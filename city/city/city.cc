...
static std::vector<City::Building> create_buildings(std::vector<BuildingConfig> const& buildings, geo::Point const& center,
    std::vector<geo::Shape> const& poisson_disks, float angle, float angle_variation, std::mt19937& rng)
{
    ...
    for (size_t i = 0; i < std::min(bs.size(), poisson_disks.size()); ++i) {
        ...
        r.emplace_back(
            geo::Shape::Box(poisson_disks.at(i).center() - geo::Point(config.w / 2.f, config.h / 2.f), { config.w, config.h }, angle + dist(rng)),
            geo::Point(0, 0)  // TODO
        );
    }
    return r;
...
City generate_city(CityConfig const& cfg)
{
    ...
    city.buildings = create_buildings(cfg.buildings, cfg.center, city.poisson_disks, cfg.angle, cfg.angle_variation, rng);
